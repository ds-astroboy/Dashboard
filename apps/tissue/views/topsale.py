# import dash
# from dash import html, dcc, dash_table, Input, Output, dcc
...
#
# # Merging data frames
# covid_data = total_confirmed.merge(right=total_deaths, how='left', on=['Province/State', 'Country/Region', 'date',
#                                                                        'Lat', 'Long'])
# covid_data = covid_data.merge(right=total_recovered, how='left', on=['Province/State', 'Country/Region', 'date',
#                                                                      'Lat', 'Long'])
#
# # Converting date column from string to proper date format
# covid_data['date'] = pd.to_datetime(covid_data['date'])
#
# # Check how many missing value is there
# covid_data.isna().sum()
#
...
#     html.Div([
#         html.Div([
#             html.Img(src="assets/Coronavirus 2019-nCoV SARS-CoV-2.E10.watermarked.2k.png",
#                      id='corona-image',
#                      style={
#                          "height": "60px",
#                          "width": "auto",
...
#                      },
#                      ),
#             html.H5("SA-BG-IT", style={"margin-top": "0px", 'color': 'orange'}),
#         ],
...
#                 html.H5("Data collected from Johns Hopkins University", style={"margin-top": "0px", 'color': 'white'}),
#             ])
#         ], className="one-half column", id="title"),
#
#         html.Div([
#             html.H6('Last Updated: ' + str(covid_data2['date'].iloc[-1].strftime("%B %d, %Y")) + '  00:01 (UTC)',
#                     style={'color': 'orange'}),
#
...
#                                        covid_data1['confirmed'].iloc[-1]) * 100, 2)) + '%)',
#                    style={
#                        'textAlign': 'center',
#                        'color': 'orange',
#                        'fontSize': 15,
...
#                     ),
#
#             html.P(f"{covid_data1['death'].iloc[-1]:,.00f}",
#                    style={
#                        'textAlign': 'center',
#                        'color': '#dd1e35',
#                        'fontSize': 40}
...
#                    + ' (' + str(round(((covid_data1['death'].iloc[-1] - covid_data1['death'].iloc[-2]) /
#                                        covid_data1['death'].iloc[-1]) * 100, 2)) + '%)',
#                    style={
#                        'textAlign': 'center',
#                        'color': '#dd1e35',
#                        'fontSize': 15,
...
#                    + ' (' + str(round(((covid_data1['recovered'].iloc[-1] - covid_data1['recovered'].iloc[-2]) /
#                                        covid_data1['recovered'].iloc[-1]) * 100, 2)) + '%)',
#                    style={
#                        'textAlign': 'center',
#                        'color': 'green',
#                        'fontSize': 15,
...
#                    + ' (' + str(round(((covid_data1['active'].iloc[-1] - covid_data1['active'].iloc[-2]) /
#                                        covid_data1['active'].iloc[-1]) * 100, 2)) + '%)',
#                    style={
#                        'textAlign': 'center',
#                        'color': '#e55467',
#                        'fontSize': 15,
...
#                         hoverinfo='text',
#                         hovertext=
#                         '<b>Date</b>: ' + covid_data_3[covid_data_3['Country/Region'] == w_countries]['date'].tail(
#                             30).astype(str) + '<br>' +
#                         '<b>Daily confirmed</b>: ' + [f'{x:,.0f}' for x in
#                                                       covid_data_3[covid_data_3['Country/Region'] == w_countries][
#                                                           'daily confirmed'].tail(30)] + '<br>' +
#                         '<b>Country</b>: ' + covid_data_3[covid_data_3['Country/Region'] == w_countries][
#                             'Country/Region'].tail(30).astype(str) + '<br>'
#
#                         ),
...
#                             '<b>Date</b>: ' + covid_data_3[covid_data_3['Country/Region'] == w_countries]['date'].tail(
#                                 30).astype(str) + '<br>' +
#                             '<b>Rolling Ave.(last 7 days)</b>: ' + [f'{x:,.0f}' for x in covid_data_3[
#                                 covid_data_3['Country/Region'] == w_countries]['Rolling Ave.'].tail(30)] + '<br>'
#                             )],
#
#         'layout': go.Layout(
#             plot_bgcolor='#1f2c56',
...
#               [Input('w_countries', 'value')])
# def update_graph(w_countries):
#     covid_data_3 = covid_data.groupby(['Lat', 'Long', 'Country/Region'])[
#         ['confirmed', 'death', 'recovered', 'active']].max().reset_index()
